...
class GridAnimation {
    ...
    trigger = () => {
        ...
        this.animate();
         // ==================== [수정된 부분] ====================
          // 가장 늦게 시작하는 조각의 지연 시간 + 기본 애니메이션 시간을 더해 총 시간을 계산
          const maxDelay = ((this.row - 1) + (this.col - 1)) * this.delayDelta;
          const totalAnimationTime = maxDelay + this.duration;

          // 총 애니메이션 시간 후에 'gridAnimationEnd' 이벤트를 발생시킴
          setTimeout(() => {
            const animationEndEvent = new CustomEvent('gridAnimationEnd', {
                bubbles: true,
                detail: {
                    // 이벤트와 함께 부모 article 요소를 전달
                    parentArticle: this.element.closest('.grid-item')
                }
            });
          this.element.dispatchEvent(animationEndEvent);
          this.element.classList.add('loaded'); // 애니메이션 완료 표시
          }, totalAnimationTime);
        // =======================================================
