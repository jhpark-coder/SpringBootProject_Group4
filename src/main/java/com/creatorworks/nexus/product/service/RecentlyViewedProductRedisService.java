package com.creatorworks.nexus.product.service; // 또는 적절한 서비스 패키지
...
public class RecentlyViewedProductRedisService {
     ...
     * @param productId 조회한 상품의 ID
     */
    public void addProductToHistory(Long memberId, Long productId) {
     ...
     */
    public List<Long> getRecentlyViewedProductIds(Long memberId, int limit) {
        String key = getKey(memberId);

        // ZSet에서 score가 높은 순(최신순)으로 상품 ID를 limit 개수만큼 가져옵니다.
        // reverseRange(key, start, end) -> start부터 end까지의 멤버를 가져옴 (0부터 시작)
        Set<String> recentProductIdsStr = redisTemplate.opsForZSet().reverseRange(key, 0, limit - 1);
        ...
        return recentProductIdsStr.stream()
                .map(Long::parseLong)
