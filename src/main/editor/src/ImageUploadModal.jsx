        ...
        if (xhr.status === 200) {
          try {
            ...
            if (imageUrl && imageUrl.trim() !== '' && imageUrl !== '[]') {
              ...
              // Base64 데이터 URL을 이미지 소스로 사용
              onImageAdd({ src: fullImageUrl, alt: selectedFile.name });
              onClose();
            } else {
              console.error('Empty or invalid response from server:', imageUrl);
              alert('이미지 업로드 중 오류가 발생했습니다. 서버에서 유효한 URL을 반환하지 않았습니다.');
  ...
  return (
    <div className="modal-overlay fancy-modal-overlay" onClick={onClose}>
      <div className="modal-content fancy-modal-content animate-fadeIn" onClick={e => e.stopPropagation()}>
        <h2 className="fancy-modal-title">이미지 업로드</h2>
        <div className="modal-section" style={{ marginBottom: '1.2rem' }}>
          <label className="fancy-label">URL로 이미지 추가</label>
        ...
        <div className="modal-section">
          ...
          <div className="fancy-file-upload-group" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            />
            <label htmlFor="image-upload-input" className="fancy-embed-btn fancy-file-btn">
              파일 선택
            </label>
