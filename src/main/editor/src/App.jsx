...
function App() {
  ...
  const handleEmbed = (urlOrIframe) => {
    ...
    if (urlOrIframe.includes('<iframe')) {
      ...
      if (!embedUrl) {
        showAlert('iframe 코드에서 src URL을 찾을 수 없습니다.');
        return;
      }
    } else {
    ...
    if (embedUrl) {
      ...
      showAlert('유효한 유튜브 URL 또는 아이프레임 코드를 입력해주세요.');
    }
  };
  ...
  const handleSaveDocument = async () => {
    ...
    if (!jsonContent || !htmlContent) {
      ...
      showAlert('에디터 내용이 비어있어 저장할 수 없습니다. 설정을 먼저 완료해주세요.');
      setIsSettingsModalOpen(true); // 설정 모달을 자동으로 엽니다.
      return;
    }

    // 2. 필수 설정값 검증
    if (!projectSettings.title || !projectSettings.tags.length || !projectSettings.saleType) {
      showAlert('프로젝트 제목, 카테고리, 판매 방식을 설정해주세요.');
      setIsSettingsModalOpen(true); // 설정 모달을 자동으로 엽니다.
      return;
    }
    ...
    // 4. 판매 방식(saleType)에 따라 API URL과 전송할 데이터(payload)를 결정합니다.
    let apiUrl = '';
    let payload = {};
    ...
    try {
      ...
      showAlert('문서 저장에 실패했습니다. 콘솔을 확인해주세요.');
    }
  };
  ...
  return (
    ...
    <div className="app-container">
      ...
      <div className="modals">
        ...
        {alertModal.isOpen && (
          <Suspense fallback={<div>알림 모달 로딩 중...</div>}>
            <AlertModal
